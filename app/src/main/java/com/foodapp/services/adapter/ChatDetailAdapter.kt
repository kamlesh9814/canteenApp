...
class ChatDetailAdapter(context: Context, list: ArrayList<Data>) :
    ...
    private inner class View2ViewHolder(itemView: View) :
        RecyclerView.ViewHolder(itemView) {
        var message: TextView = itemView.findViewById(R.id.right_Massage)
        var time: TextView = itemView.findViewById(R.id.right_Time_TV)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int):RecyclerView.ViewHolder {
        if (viewType == VIEW_TYPE_ONE) {
            return View1Holder(
                LayoutInflater.from(context).inflate(R.layout.left_chat_layout, parent, false)
            )
        }
        return View2ViewHolder(
            LayoutInflater.from(context).inflate(R.layout.right_chat_layout, parent, false)
        )
    }
